        ...
        days.map(day => (
          <div key={day}>
            <h3 className="itinerary-day-header">
              <span>📅</span> Day {day} <span>-- {dayGroups[day].length} activities</span>
            </h3>

            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {dayGroups[day].sort((a, b) => a.order - b.order).map((item, idx) => {
                const imgSrc = idx % 3 === 0 ? '/images/act_temple.png' : (idx % 3 === 1 ? '/images/act_food.png' : '/images/act_train.png');
                return (
                  <div key={item.id} className="activity-card">
                    <img src={imgSrc} alt="Activity Thumbnail" className="activity-image" />
                    <div className="activity-content">
                      ...
                      <button className="btn btn-ghost btn-sm" onClick={() => handleDelete(item.id)} style={{ color: '#ef4444', padding: '4px 0', alignSelf: 'flex-start', marginTop: '8px' }}>
                        Delete
                      </button>
                    </div>
                    <div className="activity-meta">
                      <div className="activity-meta-icon">
                        {idx % 3 === 0 ? '⛩️' : (idx % 3 === 1 ? '🍜' : '🚆')}
                      </div>
                      {item.duration_minutes ? `${item.duration_minutes}min` : '60min'}
                    </div>
                  </div>
                );
              })}
