...
IMPORTANT: Return ONLY a JSON array of activities. Do not add any extra text. Each activity must have:
...
Do not include a summary paragraph or any markdown outside the JSON array.
...
def parse_itinerary_json(text):
    ...
    def clean_json_string(s):
        ...
        s = re.sub(r',\s*\]', ']', s)
        s = re.sub(r',\s*\}', '}', s)
        # Fix common malformed patterns from models
        s = re.sub(r'"title"\s*:\s*-\s*', '"title": "', s)
        s = re.sub(r'"title"\s*,\s*-\s*', '"title": "', s)
        s = re.sub(r'"duration_minutes"\s*"+\s*', '"duration_minutes": "', s)
        s = re.sub(r'"location"\s*:\s*-\s*', '"location": "', s)
        s = re.sub(r'"start_time"\s*:\s*-\s*', '"start_time": "', s)
        return s
    ...
    # 3. Try to find a JSON object with an activities list
    start_obj = text.find('{')
    if start_obj != -1:
        end_obj = text.rfind('}')
        if end_obj > start_obj:
            try:
                parsed = json.loads(clean_json_string(text[start_obj:end_obj+1]))
                if isinstance(parsed, dict) and isinstance(parsed.get('activities'), list):
                    return parsed['activities']
            except json.JSONDecodeError:
                pass

    # 4. Lenient fallback: extract object blocks and parse individually
    activities = []
    for obj_match in re.findall(r'\{[\s\S]*?\}', text):
        try:
            cleaned = clean_json_string(obj_match)
...
def generate_itinerary(destination, duration_days, budget=None, currency='EUR', preferences=''):
    """Generate a full itinerary in one request and return activities + raw text."""
    try:
        ...
        return {'activities': activities, 'full_text': full_text}
    except Exception as e:
        return {'error': str(e), 'activities': [], 'full_text': ''}
